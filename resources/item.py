...
class Item(Resource):
    ...
    def put(self, name):
        ...
        item = ItemModel.find_by_name(name) # retrieved from database
...
class ItemList(Resource):
    def get(self):      
        #return {'items': list(map(lambda x: x.json(), ItemModel.find_all(self)))}
        return {'items': [x.json() for x in ItemModel.find_all(self)]}